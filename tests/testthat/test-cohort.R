test_that("cohort = min(year) when treated, 0 otherwise", {
  ...
  ch <- rflexdid:::make_cohort(group, time, tx)
...
test_that("validate_binary catches non-binary tx", {
  expect_error(rflexdid:::validate_binary(c(0, 1, 2), "tx"), "binary")
  expect_error(rflexdid:::validate_binary(c(1, 1, 1), "tx"), "binary")
  expect_invisible(rflexdid:::validate_binary(c(0L, 1L, 0L, 1L), "tx"))
})

test_that("check_time_gaps requires usercohort when gaps present", {
  expect_invisible(rflexdid:::check_time_gaps(2030:2034, FALSE))
  expect_error(rflexdid:::check_time_gaps(c(2030, 2031, 2033), FALSE),
               "gaps")
  # OK if usercohort supplied
  expect_invisible(rflexdid:::check_time_gaps(c(2030, 2031, 2033), TRUE))
})
...
test_that("make_tx_indicator sets _Tx=1 in pre-period for treated cohorts", {
  ...
  out <- rflexdid:::make_tx_indicator(tx, cohort, time)
...
test_that("handle_treatment_status drops last cohort when no never-treated", {
  ...
  res <- rflexdid:::handle_treatment_status(cohort, time, verbose = FALSE)
...
test_that("handle_treatment_status errors on always-treated", {
  ...
  expect_error(rflexdid:::handle_treatment_status(cohort, time, verbose = FALSE),
               "always-treated")
})
